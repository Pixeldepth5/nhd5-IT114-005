...
public class Problem2 extends BaseClass {
    ...
    private static void sumValues(double[] arr, int arrayNumber) {
        ...
        // ndh5, 10/01/25
        // For Challenege 1 
        //  Step 1 : Iterate through the array by using a for-each loop (for(double value : arr))
        //  Step 2 : track sum by adding each value into the running variable 'total' (total = total + value)
        //  Step 3 : update representation by using String.format("%.2f", total) and assign to modifiedTotal - from W3 Schools

          for (double value : arr) {
            total = total + value;   // add each element to the running total
        }
      
        // Solve Challenge 2 here
        // For Chhallenge 2 
        //  Step 1 : create 2 decimal string from total using String.format ("%.2f", total)
        //    source: W3Schools — String format(): https://www.w3schools.com/java/ref_string_format.asp
        // Step 2 : assign string to ModifiedTotal (object can hold string)
        // Step 3 : print the output to show the exatct two decimals places
